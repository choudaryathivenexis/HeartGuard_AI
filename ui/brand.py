...
def wordmark(size: int = 18, dark: bool = False) -> str:
    """`HeartGuard` in Archivo 600 expanded, then `AI` in Verdigris behind a rule."""
...
def lockup(size: int = 26, dark: bool = False, wordmark_size: int = 18) -> str:
    """
    ...
    return (
        ...
        f'color:{T.CSS_DARK["text_heading"] if dark else T.CSS["text_heading"]};">'
...
def favicon_path(dark: bool = False) -> str | None:
    """Path for st.set_page_config(page_icon=...), or None if generation failed."""
    generate_favicons()
...
def export_lockup_svg() -> str | None:
    ...
    svg = (
        ...
        f'<rect x="166" y="11" width="1" height="16" fill="{T.VERDIGRIS}"/>'
        f'<text x="175" y="26" font-family="Archivo, sans-serif" font-size="14.4" '
        f'font-weight="400" fill="{T.VERDIGRIS}">AI</text>'
        '</svg>'
    )
    try:
