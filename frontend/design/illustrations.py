...
WHY SVG, AND WHERE THAT NO LONGER APPLIES
    Vector artwork is generated from ui/tokens.py, so it re-themes automatically, costs
    almost nothing on disk, stays sharp at any viewport, and cannot 404 on a machine
    with no internet — the exact condition a marker may run this under. That is why
    every mark, icon and trace in this application is drawn rather than drawn ON.

    ONE EXCEPTION NOW SHIPS: the sign-in panel is a supplied image,
    frontend/static/img/auth-hero.webp, applied as a CSS background. It was chosen
    deliberately by the project owner, and the reasons above are not violated by it —
    the file is served from this origin, so it needs no network and cannot 404. The
    `auth_backdrop()` function that used to draw that panel was deleted rather than
    left in place: two ways to render the same surface is how one of them silently
